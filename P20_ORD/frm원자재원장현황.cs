...
{
    public partial class frm원자재원장현황 : Form
    {
        ...
        private void frm원자재원장현황_Load(object sender, EventArgs e)
        {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    MessageBox.Show("조회할 원부자재를 선택해주십시오");
                    return;
                }
                in_grid_logic();
                calBalstock_logic();
        ...
        private void in_grade_logic()
        {
            ...
            dt = wDm.fn_Sales_Detail_Order_Count_Only_Raw(sb.ToString());
        ...
        {
            Popup.pop원부재료검색 msg = new Popup.pop원부재료검색();
        ...
        private void input_raw_list()
        {
            try
            {
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    txt_raw_cd.Text = dt.Rows[0]["원부재료코드"].ToString();
                    txt_raw_nm.Text = dt.Rows[0]["원부재료명"].ToString();
                    txt_raw_gubun.Text = dt.Rows[0]["원부재료구분"].ToString();
        ...
        private void inputRmGrid_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex > -1)
            {
                    ...
                    && !inputRmGrid.Rows[e.RowIndex].Cells["UNION_CD"].Value.ToString().Equals(""))
                {
                    ...
                    form.StartPosition = FormStartPosition.CenterParent;
                    form.Show();
                    form.Srch_by_union(form.SrchValue);
