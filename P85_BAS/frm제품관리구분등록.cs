...
{
    public partial class frm제품관리구분등록 : Form
    {
        ...
        private void frm제품관리구분등록_Load(object sender, EventArgs e)
        {
        ...
        private void bindData(string condition)
        {
            ...
            try
            {
                wnDm wDm = new wnDm();
                DataTable dt = null;
                dt = wDm.fn_PRODUCTCLASS_List(condition);
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    {
                        this.GridRecord.Rows[i].Cells[0].Value = dt.Rows[i]["PRODUCT_CLASS"].ToString();
        ...
        private void getDetailPost(string sKey)
        {
            ...
            try
            {
                wnDm wDm = new wnDm();
                DataTable dt = null;
                dt = wDm.fn_PRODUCTCLASS_Detail(sKey);

                if (dt != null && dt.Rows.Count > 0)
                {
                    this.txtCode.Text = dt.Rows[0]["PRODUCT_CLASS"].ToString();
        ...
        private bool validate_InputBox()
        {
            ...
            try
            {
                ...
                {
                    MessageBox.Show("[ 제품관리구분명 ]을 입력하세요.");
                    return false;
                }
        ...
        private bool insertPost()
        {
            ...
            try
            {
                ...
                sb.AppendLine(" insert into PRODUCTCLASS ( ");
                sb.AppendLine("     PRODUCT_CLASS ");
                ...
                sCommand.Parameters.AddWithValue("@PRODUCT_CLASS", txtCode.Text);
                sCommand.Parameters.AddWithValue("@CODE_DESC", txtName.Text);

                int qResult = wAdo.SqlCommandEtc(sCommand, "Insert PRODUCTCLASS");
        ...
        private bool updatePost()
        {
            ...
            try
            {
                ...
                sb.AppendLine(" update PRODUCTCLASS set ");
                sb.AppendLine("     CODE_DESC = @CODE_DESC ");
                sb.AppendLine(" where 1=1 ");
                sb.AppendLine("     and PRODUCT_CLASS = @p1 ");
                ...
                int qResult = wAdo.SqlCommandEtc(sCommand, "Update PRODUCTCLASS");
                if (qResult > 0) bRet = true;
                else bRet = false;
        ...
        private bool deletePost()
        {
            ...
            try
            {
                ...
                sb.AppendLine(" delete from PRODUCTCLASS ");
                sb.AppendLine(" where PRODUCT_CLASS = @p1  ");

                SqlCommand sCommand = new SqlCommand(sb.ToString());

                sCommand.Parameters.AddWithValue("@p1", this.txtCode.Text);

                int qResult = wAdo.SqlCommandEtc(sCommand, "Delete PRODUCTCLASS");
                if (qResult > 0) bRet = true;
                else bRet = false;
        ...
        private bool get_Dup_Check(string sNew, string sOld)
        {
            try
            {
                if (sNew != sOld)
                {
                    wnDm wDm = new wnDm();
                    DataTable dt = null;
                    dt = wDm.fn_PRODUCTCLASS_Detail(sNew);
