...
{
    public partial class frm제형구분등록 : Form
    {
        ...
        private void frm제형구분등록_Load(object sender, EventArgs e)
        {
        ...
        private void bindData(string condition)
        {
            ...
            try
            {
                wnDm wDm = new wnDm();
                DataTable dt = null;
                dt = wDm.fn_PRODUCTTYPE_List(condition);
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    {
                        this.GridRecord.Rows[i].Cells[0].Value = dt.Rows[i]["PRODUCT_TYPE"].ToString();
        ...
        private void getDetailPost(string sKey)
        {
            ...
            try
            {
                wnDm wDm = new wnDm();
                DataTable dt = null;
                dt = wDm.fn_PRODUCTTYPE_Detail(sKey);

                if (dt != null && dt.Rows.Count > 0)
                {
                    this.txtCode.Text = dt.Rows[0]["PRODUCT_TYPE"].ToString();
        ...
        private bool insertPost()
        {
            ...
            try
            {
                ...
                sb.AppendLine(" insert into PRODUCTTYPE ( ");
                ...
                sCommand.Parameters.AddWithValue("@PRODUCT_TYPE", txtCode.Text);
                sCommand.Parameters.AddWithValue("@CODE_DESC", txtName.Text);

                int qResult = wAdo.SqlCommandEtc(sCommand, "Insert PRODUCTTYPE");
        ...
        private bool updatePost()
        {
            ...
            try
            {
                ...
                sb.AppendLine(" update PRODUCTTYPE set ");
                sb.AppendLine("     CODE_DESC = @CODE_DESC ");
                sb.AppendLine(" where 1=1 ");
                sb.AppendLine("     and PRODUCT_TYPE = @p1 ");
                ...
                int qResult = wAdo.SqlCommandEtc(sCommand, "Update PRODUCTTYPE");
                if (qResult > 0) bRet = true;
                else bRet = false;
        ...
        private bool deletePost()
        {
            ...
            try
            {
                ...
                sb.AppendLine(" delete from PRODUCTTYPE ");
                sb.AppendLine(" where PRODUCT_TYPE = @p1  ");

                SqlCommand sCommand = new SqlCommand(sb.ToString());

                sCommand.Parameters.AddWithValue("@p1", this.txtCode.Text);

                int qResult = wAdo.SqlCommandEtc(sCommand, "Delete PRODUCTTYPE");
                if (qResult > 0) bRet = true;
                else bRet = false;
        ...
        private bool get_Dup_Check(string sNew, string sOld)
        {
            try
            {
                if (sNew != sOld)
                {
                    wnDm wDm = new wnDm();
                    DataTable dt = null;
                    dt = wDm.fn_PRODUCTTYPE_Detail(sNew);
