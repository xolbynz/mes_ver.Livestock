...
{
    public partial class frm창고등록 : Form
    {
        ...
        private void frm창고등록_Load(object sender, EventArgs e)
        {
        ...
        private string makeSearchCondition()
        {
            ...
            switch (this.txtSrch.Text)
            {
                case "":
                    ...
                    break;
                default:
                    sb.Append(" and a.STORAGE_NM like '%" + txtSrch.Text + "%' ");
                    break;
            }
        ...
        private void getDetailPost(string sKey)
        {
            ...
            try
            {
                wnDm wDm = new wnDm();
                DataTable dt = null;
                dt = wDm.fn_STORECODE_Detail(sKey);

                if (dt != null && dt.Rows.Count > 0)
                {
                    this.txtCode.Text = dt.Rows[0]["STORAGE_CD"].ToString();
                    this.txtName.Text = dt.Rows[0]["STORAGE_NM"].ToString();
                }
                else
                {
        ...
        private bool insertPost()
        {
            ...
            try
            {
                ...
                sb.AppendLine("     @STORE_CODE ");
                sb.AppendLine("     , @CODE_DESC ");
                sb.AppendLine(" ) ");

                SqlCommand sCommand = new SqlCommand(sb.ToString());

                sCommand.Parameters.AddWithValue("@STORE_CODE", txtCode.Text);
                sCommand.Parameters.AddWithValue("@CODE_DESC", txtName.Text);

                int qResult = wAdo.SqlCommandEtc(sCommand, "Insert STORECODE");
        ...
        private bool updatePost()
        {
            ...
            try
            {
                ...
                sb.AppendLine("     STORAGE_NM = @CODE_DESC ");
                sb.AppendLine(" where 1=1 ");
                sb.AppendLine("     and STORAGE_CD = @p1 ");
                ...
                int qResult = wAdo.SqlCommandEtc(sCommand, "Update STORECODE");
                if (qResult > 0) bRet = true;
                else bRet = false;
        ...
        private bool deletePost()
        {
            ...
            try
            {
                ...
                sb.AppendLine(" where STORAGE_CD = @p1  ");

                SqlCommand sCommand = new SqlCommand(sb.ToString());

                sCommand.Parameters.AddWithValue("@p1", this.txtCode.Text);

                int qResult = wAdo.SqlCommandEtc(sCommand, "Delete STORECODE");
                if (qResult > 0) bRet = true;
                else bRet = false;
        ...
        private bool get_Dup_Check(string sNew, string sOld)
        {
            try
            {
                if (sNew != sOld)
                {
                    wnDm wDm = new wnDm();
                    DataTable dt = null;
                    dt = wDm.fn_STORECODE_Detail(sNew);
