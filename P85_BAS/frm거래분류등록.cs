...
{
    public partial class frm거래분류등록 : Form
    {
        ...
        private void frm거래분류등록_Load(object sender, EventArgs e)
        {
        ...
        private void bindData(string condition)
        {
            ...
            try
            {
                wnDm wDm = new wnDm();
                DataTable dt = null;
                dt = wDm.fn_DEALKIND_List(condition);
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    {
                        this.GridRecord.Rows[i].Cells[0].Value = dt.Rows[i]["DEAL_KIND"].ToString();
        ...
        private void getDetailPost(string sKey)
        {
            ...
            try
            {
                wnDm wDm = new wnDm();
                DataTable dt = null;
                dt = wDm.fn_DEALKIND_Detail(sKey);

                if (dt != null && dt.Rows.Count > 0)
                {
                    this.txtCode.Text = dt.Rows[0]["DEAL_KIND"].ToString();
        ...
        private bool insertPost()
        {
            ...
            try
            {
                ...
                sb.AppendLine(" insert into DEALKIND ( ");
                ...
                sCommand.Parameters.AddWithValue("@DEAL_KIND", txtCode.Text);
                sCommand.Parameters.AddWithValue("@CODE_DESC", txtName.Text);

                int qResult = wAdo.SqlCommandEtc(sCommand, "Insert DEALKIND");
        ...
        private bool updatePost()
        {
            ...
            try
            {
                ...
                sb.AppendLine(" update DEALKIND set ");
                sb.AppendLine("     CODE_DESC = @CODE_DESC ");
                sb.AppendLine(" where 1=1 ");
                sb.AppendLine("     and DEAL_KIND = @p1 ");
                ...
                int qResult = wAdo.SqlCommandEtc(sCommand, "Update DEALKIND");
                if (qResult > 0) bRet = true;
                else bRet = false;
        ...
        private bool deletePost()
        {
            ...
            try
            {
                ...
                sb.AppendLine(" delete from DEALKIND ");
                sb.AppendLine(" where DEAL_KIND = @p1  ");

                SqlCommand sCommand = new SqlCommand(sb.ToString());

                sCommand.Parameters.AddWithValue("@p1", this.txtCode.Text);

                int qResult = wAdo.SqlCommandEtc(sCommand, "Delete DEALKIND");
                if (qResult > 0) bRet = true;
                else bRet = false;
        ...
        private bool get_Dup_Check(string sNew, string sOld)
        {
            try
            {
                if (sNew != sOld)
                {
                    wnDm wDm = new wnDm();
                    DataTable dt = null;
                    dt = wDm.fn_DEALKIND_Detail(sNew);
