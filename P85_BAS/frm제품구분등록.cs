...
{
    public partial class frm제품구분등록 : Form
    {
        ...
        private void frm제품구분등록_Load(object sender, EventArgs e)
        {
        ...
        private void bindData(string condition)
        {
            ...
            try
            {
                wnDm wDm = new wnDm();
                DataTable dt = null;
                dt = wDm.fn_PRODUCTKIND_List(condition);
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    {
                        this.GridRecord.Rows[i].Cells[0].Value = dt.Rows[i]["PRODUCT_KIND"].ToString();
        ...
        private void getDetailPost(string sKey)
        {
            ...
            try
            {
                wnDm wDm = new wnDm();
                DataTable dt = null;
                dt = wDm.fn_PRODUCTKIND_Detail(sKey);

                if (dt != null && dt.Rows.Count > 0)
                {
                    this.txtCode.Text = dt.Rows[0]["PRODUCT_KIND"].ToString();
        ...
        private bool updatePost()
        {
            ...
            try
            {
                ...
                sb.AppendLine(" update PRODUCTKIND set ");
                sb.AppendLine("     CODE_DESC = @CODE_DESC ");
                sb.AppendLine(" where 1=1 ");
                sb.AppendLine("     and PRODUCT_KIND = @p1 ");
                ...
                int qResult = wAdo.SqlCommandEtc(sCommand, "Update PRODUCTKIND");
                if (qResult > 0) bRet = true;
                else bRet = false;
        ...
        private bool deletePost()
        {
            ...
            try
            {
                ...
                sb.AppendLine(" delete from PRODUCTKIND ");
                sb.AppendLine(" where PRODUCT_KIND = @p1  ");

                SqlCommand sCommand = new SqlCommand(sb.ToString());

                sCommand.Parameters.AddWithValue("@p1", this.txtCode.Text);

                int qResult = wAdo.SqlCommandEtc(sCommand, "Delete PRODUCTKIND");
                if (qResult > 0) bRet = true;
                else bRet = false;
        ...
        private bool get_Dup_Check(string sNew, string sOld)
        {
            try
            {
                if (sNew != sOld)
                {
                    wnDm wDm = new wnDm();
                    DataTable dt = null;
                    dt = wDm.fn_PRODUCTKIND_Detail(sNew);
