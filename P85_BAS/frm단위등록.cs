...
{
    public partial class frm단위등록 : Form
    {
        ...
        private void frm단위등록_Load(object sender, EventArgs e)
        {
        ...
        private void bindData(string condition)
        {
            ...
            try
            {
                wnDm wDm = new wnDm();
                DataTable dt = null;
                dt = wDm.fn_UnitCode_List(condition);
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    {
                        this.GridRecord.Rows[i].Cells[0].Value = dt.Rows[i]["UNIT_CODE"].ToString();
        ...
        private void getDetailPost(string sKey)
        {
            ...
            try
            {
                wnDm wDm = new wnDm();
                DataTable dt = null;
                dt = wDm.fn_UnitCode_Detail(sKey);

                if (dt != null && dt.Rows.Count > 0)
                {
                    this.txtCode.Text = dt.Rows[0]["UNIT_CODE"].ToString();
        ...
        private bool insertPost()
        {
            ...
            try
            {
                ...
                sb.AppendLine(" insert into UnitCode ( ");
                ...
                sCommand.Parameters.AddWithValue("@UNIT_CODE", txtCode.Text);
                sCommand.Parameters.AddWithValue("@CODE_DESC", txtName.Text);

                int qResult = wAdo.SqlCommandEtc(sCommand, "Insert UnitCode");
        ...
        private bool updatePost()
        {
            ...
            try
            {
                ...
                sb.AppendLine(" update UnitCode set ");
                sb.AppendLine("     CODE_DESC = @CODE_DESC ");
                sb.AppendLine(" where 1=1 ");
                sb.AppendLine("     and UNIT_CODE = @p1 ");
                ...
                int qResult = wAdo.SqlCommandEtc(sCommand, "Update UnitCode");
                if (qResult > 0) bRet = true;
                else bRet = false;
        ...
        private bool deletePost()
        {
            ...
            try
            {
                ...
                sb.AppendLine(" delete from UnitCode ");
                sb.AppendLine(" where UNIT_CODE = @p1  ");

                SqlCommand sCommand = new SqlCommand(sb.ToString());

                sCommand.Parameters.AddWithValue("@p1", this.txtCode.Text);

                int qResult = wAdo.SqlCommandEtc(sCommand, "Delete UnitCode");
                if (qResult > 0) bRet = true;
                else bRet = false;
        ...
        private bool get_Dup_Check(string sNew, string sOld)
        {
            try
            {
                if (sNew != sOld)
                {
                    wnDm wDm = new wnDm();
                    DataTable dt = null;
                    dt = wDm.fn_UnitCode_Detail(sNew);
