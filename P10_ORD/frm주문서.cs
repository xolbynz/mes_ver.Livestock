...
{
    public partial class frm주문서 : Form
    {
        ...
        private string sSettingFile = "order.xml";
        ...
        private void frm주문서_Load(object sender, EventArgs e)
        {
        ...
        private void bindData(string condition)
        {
            ...
            try
            {
                wnDm wDm = new wnDm();
                DataTable dt = null;
                dt = wDm.fn_PRODUCT_STOCK_List(condition);
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        this.GridRecord.Rows[i].Cells[7].Value = dt.Rows[i]["TAX_DATE"].ToString().Trim();
                    }
                }
            }
            catch (Exception ex)
        ...
        private void getDetailPost(string sKey, string sKey2)
        {
            ...
            try
            {
                wnDm wDm = new wnDm();
                DataTable dt = null;
                dt = wDm.fn_PRODUCT_STOCK_Detail(sKey, sKey2);
                ...
                if (dt != null && dt.Rows.Count > 0)
                {
                    ...
                    // 주문서에서 저장시 '' 이고 , 미송 마감시에만 체크함(1)  : 현재 의미는 없어짐
                    if (dt.Rows[0]["CHK_STOCK1"].ToString().Trim() == "1")
                    {
                        chk미송정리.Checked = true;
                    }
                    else
                    {
                        chk미송정리.Checked = false;
                    }
            ...
            try
            {
                ...
                for (int kk = 0; kk < grdItem.Rows.Count; kk++)
                {
                    if ("" + (string)grdItem.Rows[kk].Cells[4].Value != "")
                    {
                        ...
                        {
                            if (txt거래처코드.Text == "110114" || txt거래처코드.Text == "110115" || txt거래처코드.Text == "110116" || txt거래처코드.Text == "110117" || txt거래처코드.Text == "110118")
                            {
                                // 거래처가 110114(샘플), 110115(기부), 110116 (클레임), 110117 (폐기), 수출(110118) 이면 등록 가능
                            }
                            else
                            {
                                bZeroChk = true;
            ...
            try
            {
                ...
                for (int kk = 0; kk < grdItem.Rows.Count; kk++)
                {
                    ...
                    if (s코드 != "")
                    {
                        ...
                        sb.AppendLine("     , '0' "); //CHK_STOCK
                        sb.AppendLine("     , '0' "); //CHK_STOCK1 : 미송정리
                        sb.AppendLine("     , NULL "); //CHK_PRT
                        sb.AppendLine("     , NULL "); //TAX_DATE
                        sb.AppendLine("     , NULL "); //TAX_NUM
                ...
                sb.AppendLine("		, null ");
                sb.AppendLine("		, '' ");
                sb.AppendLine("		, b.PRODUCT_NAME ");
                sb.AppendLine("		, b.PRODUCT_SPEC ");
                ...
                sb.AppendLine("		and a.STOCK_NUM = @maxKey ");

                sb.AppendLine(" update NUM_STOCK set STOCK_NUM = @maxKey where STOCK_DATE = @STOCK_DATE ");

                SqlCommand sCommand = new SqlCommand(sb.ToString());

                sCommand.Parameters.AddWithValue("@STOCK_DATE", dtp일자.Text);

                int qResult = wAdo.SqlCommandEtc(sCommand, "Insert-PRODUCT_STOCK");
        ...
        private bool updatePost()
        {
            ...
            try
            {
                ...
                sb.AppendLine(" update PRODUCT_STOCK set ");
                sb.AppendLine("     STOCK_ILNUM = -1 ");
                sb.AppendLine(" where STOCK_DATE = @STOCK_DATE ");
                sb.AppendLine("     and STOCK_NUM = @p1 ");

                ////sb.AppendLine(" delete from PRODUCT_STOCK ");
                ////sb.AppendLine(" where STOCK_DATE = @STOCK_DATE ");
                ////sb.AppendLine("     and STOCK_NUM = @p1 ");

                ////sb.AppendLine(" delete from STORE_STOCK ");
                ////sb.AppendLine(" where STOCK_DATE = @STOCK_DATE ");
                ////sb.AppendLine("     and STOCK_NUM = @p1 ");
                ...
                for (int kk = 0; kk < grdItem.Rows.Count; kk++)
                {
                    ...
                    if (s코드 != "")
                    {
                        ...
                        sb.AppendLine(" select @cnt = count(*) from PRODUCT_STOCK ");
                        sb.AppendLine(" where STOCK_DATE = @STOCK_DATE ");
                        sb.AppendLine("     and STOCK_NUM = @p1 ");
                        ...
                        sb.AppendLine("		and a.STOCK_CODE = '" + s코드 + "' ");
                        ...
                        // ====================================================================================
                        sb.AppendLine(" update STORE_STOCK set ");
                        sb.AppendLine("		STOCK_KIND = '" + cmb구분.SelectedValue.ToString() + "' ");
                        if ("" + cmb구분.SelectedValue.ToString() == "22")
                        {
                            //반품
                            sb.AppendLine("     , STOCK_QTY = -1 * " + s수량 + " ");
                            sb.AppendLine("     , STOCK_PRICE = " + s단가 + " ");
                        ...
                        sb.AppendLine("		, CUST_CODE = '" + txt거래처코드.Text + "' ");
                        sb.AppendLine("		, PRODUCT_NAME = '" + s명칭 + "' ");
                        sb.AppendLine("		, PRODUCT_SPEC = '" + s규격 + "' ");
                        sb.AppendLine("		, PRODUCT_UNIT = ( select CODE_DESC from UnitCode where UNIT_CODE = (select UNIT_CODE from PRODUCT where PRODUCT_CODE = '" + s코드 + "' ))");
                        sb.AppendLine(" where STOCK_DATE = @STOCK_DATE ");
                        sb.AppendLine("     and STOCK_NUM = @p1 ");
                        sb.AppendLine("     and STOCK_CODE = '" + s코드 + "' ");
                ...
                int qResult = wAdo.SqlCommandEtc(sCommand, "Update-PRODUCT_STOCK");
                if (qResult > 0) bRet = true;
                else bRet = false;
        ...
        private bool deletePost()
        {
            ...
            try
            {
                ...
                int qResult = wAdo.SqlCommandEtc(sCommand, "Delete-PRODUCT_STOCK");
                if (qResult > 0) bRet = true;
                else bRet = false;
